...
def dataset_load(args):
    ...
    if args.office31 == True:  # and not args.home and not args.visda:
        ...
        s_tr, s_ts = "./data/birds31/{}.txt".format(
            s
        ), "./data/birds31/{}.txt".format(s)
        ...
        prefix = '.'
        t_tr, t_ts = "./data/birds31/{}.txt".format(
            t
        ), "./data/birds31/{}.txt".format(t)
        ...
        prep_dict = {}
        prep_dict["source"] = image_train(resize_size=512, crop_size=448)
        prep_dict["target"] = image_target(resize_size=512, crop_size=448)
        prep_dict["test"] = image_test(resize_size=512, crop_size=448)
        train_source = ImageList(s_tr, transform=prep_dict["source"])
        test_source = ImageList(s_ts, transform=prep_dict["source"])
        ...
        s_tr, s_ts = "./data/cub/{}.txt".format(
            s
        ), "./data/cub/{}.txt".format(s)

        txt_src = open(s_tr).readlines()
        s_tr = txt_src
        s_ts = txt_src
        #
        prefix = './data/cub/{}/'.format(s)
        s_tr = [prefix + line.strip() for line in s_tr]
        s_ts = [prefix + line.strip() for line in s_ts]

        prefix = './data/cub/{}/'.format(t)
        t_tr, t_ts = "./data/cub/{}.txt".format(
            t
        ), "./data/cub/{}.txt".format(t)
